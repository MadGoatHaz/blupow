...
class BluPowConfigFlow(ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(
        ...
        if user_input is not None:
            ...
            name = self._discovered_devices.get(address, "Unknown Device")
        ...
        for discovery in current_discoveries:
            if (
                discovery.name.upper().startswith("REGO")
                or discovery.name.upper().startswith("RENOGY")
                or discovery.name.upper().startswith("BT-")
            ):
