...
| **Tool Failure on Directories** | Using the `delete_file` tool on a directory path instead of a file path, causing the tool to fail. | The `delete_file` tool is for files only. To remove a directory, first list its contents (`list_dir`) and then call `delete_file` on each individual file. This prevents tool errors and ensures a clean removal. |
| **Incorrect Assumption from File Listing** | Concluding a directory was missing (`/app/config`) based only on a `list_dir` result, without checking the code that might create it. | The filesystem is not always static. Always cross-reference file/directory listings with the code that uses them. The `device_manager.py` code included an `os.makedirs` call, which made the initial assumption incorrect and wasted time. Trust the code, but verify by reading it. |
